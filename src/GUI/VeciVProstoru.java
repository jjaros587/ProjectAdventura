    ...
    */ 
    ...
    public void update() {
        ...
        for (String nazevVeci : hra.getHerniPlan().getAktualniProstor().getVeci()) {
            ...
            if(hra.getHerniPlan().getAktualniProstor().vyberVec(nazevVeci).getLzeProzkoumat()){
                Button prozkoumat = new Button("Prozkoumat");
                prozkoumat.setOnAction((ActionEvent event) -> {
                    String prikaz = "prozkoumej " + nazevVeci;
                    centralText.appendText("\n" + prikaz);
                    String odpoved = hra.zpracujPrikaz(prikaz);
                    centralText.appendText("\n\n" + odpoved + "\n");
                });      
                polozka.getChildren().add(prozkoumat);
            }
