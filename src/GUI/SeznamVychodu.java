    ...
    */
    ...
    public void update() {
        ...
        for (Prostor prostor : hra.getHerniPlan().getAktualniProstor().getVychody()) {
            ...
            if (prostor.isViditelna()) {
                String text = prostor.getNazev();
                if(prostor.jeZamceno()){
                    text += " (zamceno)";                   
                }
                ...
                if(prostor.jeZamceno() & prostor.getLzeOdemknoutHracem()){
                    Button odemknout = new Button("Odemknout");
                    odemknout.setMaxWidth(Double.MAX_VALUE);
                    odemknout.setOnAction((ActionEvent event) -> {
                        String prikaz = "odemkni " + prostor.getNazev();
                        centralText.appendText("\n" + prikaz);
                        String odpoved = hra.zpracujPrikaz(prikaz);
                        centralText.appendText("\n\n" + odpoved + "\n");
                    });
                    polozka.getChildren().add(odemknout);                   
                }              
